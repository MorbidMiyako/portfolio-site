...
Made a lot of progresse, and spend a long time figuring out how to slow down the animation to a comfortable speed.
However while testing I noticed having it scroll up automatically was pretty user-UN-friandly so accepted that it wasnt the best idea...
...
function App() {
    ...
    const upScrollRef = React.createRef();
    const downScrollRef = React.createRef();


    const handleScroll = (e) => {
        if (0 < window.scrollY && window.scrollY < window.innerHeight && scrollToggle === true) {
            console.log(upScrollRef)
    ...
    // findDOMNode(ref.current).addEventListener('scroll', handleScroll())

    return (
        <div data-testid="mainDiv" style={{ backgroundColor: "black" }}>
            <div onScroll={handleScroll} ref={upScrollRef} >
                <p>hey</p>
            </div>
            {stars.map((star, id) => {
                const starStyling = {
            ...
            <div data-testid="frontPage">
                ...
                <img data-testid="scrollDownButton" src={scrollDownButtonImg} alt="scrollDownButton" onClick={handleClick} />
            </div>


            <div data-testid="aboutPage" ref={downScrollRef}>
                <h2>About me</h2>
                <p>
